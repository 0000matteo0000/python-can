...
# Module Imports
#
from ctypes import (Structure, c_char, c_char_p, c_int, c_ubyte, c_uint,
                    c_ulonglong, c_ushort, create_string_buffer)
...
LOG_FUNCTION_DEFAULT = int(0x00)  # Logs system except Exceptionions / errors
...
class PCANBasic:
    ...
    def __init__(self):
        ...
        if platform.system() == "Windows":
            ...
            self.__m_dllBasic = None  # windll.LoadLibrary("PCANBasic")
            ...
            self.__m_dllBasic = None  # cdll.LoadLibrary("PCANBasic.dll")
            # Unfortunately cygwin python has no winreg module, so we can't
            # check for the registry key.
        elif platform.system() == "Linux":
            # Loads the API on Linux
            self.__m_dllBasic = None  # cdll.LoadLibrary("libpcanbasic.so")
        elif platform.system() == "Darwin":
            self.__m_dllBasic = None  # cdll.LoadLibrary(find_library("libPCBUSB.dylib"))
        else:
            self.__m_dllBasic = None  # cdll.LoadLibrary("libpcanbasic.so")
