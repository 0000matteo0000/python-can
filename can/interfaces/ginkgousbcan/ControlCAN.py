...
import os
import platform
from ctypes import (CDLL, CFUNCTYPE, WINFUNCTYPE, Structure, WinDLL, c_byte,
...
if(platform.system() == "Windows"):
    if "64bit" in platform.architecture():
        GinkgoLib = WinDLL(os.path.dirname(__file__) + "\\lib\\windows\\64bit\\Ginkgo_Driver.dll")
    else:
        GinkgoLib = WinDLL(os.path.dirname(__file__) + "\\lib\\windows\\32bit\\Ginkgo_Driver.dll")
elif(platform.system() == "Darwin"):
    GinkgoLib = CDLL(os.path.dirname(__file__) + "/lib/macos/libGinkgo_Driver.dylib")
elif(platform.system() == "Linux"):
    if "64bit" in platform.architecture():
        GinkgoLib = CDLL(os.path.dirname(__file__) + "/lib/linux/64bit/libGinkgo_Driver.so")
    else:
        GinkgoLib = CDLL(os.path.dirname(__file__) + "/lib/linux/32bit/libGinkgo_Driver.so")
else:
    print("Unknown system", flush=True)
...
def VCI_OpenDevice(DevType, DevIndex, Reserved):
    try:
        ...
        print('%s: %s' % (args.__class__.__name__, args), flush=True)
        exit()
